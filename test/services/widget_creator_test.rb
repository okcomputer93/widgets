...
class WidgetCreatorTet < ActiveSupport::TestCase
  setup do
  ...
  test "widget names must be 5 characters or greater" do
    ...
    too_short_error = result.
      widget.
      errors[:name].
      detect { |message| message =~ /is too short/i }

    assert_not_nil too_short_error, result.widget.errors.full_messages.join(",")
  end
  ...
  test "name, price and manufacturer are required" do
    ...
    assert widget.errors[:name].any? { |message| message =~ /can't be blank/i }, widget.errors.full_messages_for(:name)
    assert widget.errors[:price_cents].any? { |message| message =~ /is not a number/i }, widget.errors.full_messages_for(:price_cents)
    assert widget.errors[:manufacturer].any? { |message| message =~ /must exist/i }, widget.errors.full_messages_for(:manufacturer)
  end
  ...
  test "price cannot be 0" do
    ...
    assert result.widget.errors[:price_cents].any? { |message| message =~ /greater than 0/i }, result.widget.errors.full_messages_for(:price_cents)
  end
  ...
  test "price cannot be more than 10,000" do
    ...
    assert result.widget.errors[:price_cents].any? { |message| message =~ /less than or equal to 100000/i }, result.widget.errors.full_messages_for(:price_cents)
  end
  ...
  test "legacy manufacturers cannot have a price under $100" do
    ...
    assert result.widget.errors[:price_cents].any? { |message| message =~ /< \$100.*legacy/i }, result.widget.errors.full_messages_for(:price_cents)
  end

  test "email admin staff for widgets on new manufacturers" do
    new_manufacturer = FactoryBot.create(:manufacturer, name: "Cyberdine Systems", created_at: 59.days.ago)
